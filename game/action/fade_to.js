...
FadeTo.create = function(to, duration, ease, callback) {
    return (caches.length ? caches.pop() : new FadeTo).init(to, duration, ease, callback)
}

FadeTo.createWithFn = function(fn, duration, ease, callback) {
    return (caches.length ? caches.pop() : new FadeTo).initWithFn(fn, duration, ease, callback)
}

FadeTo.collect = function(action) {
    caches.push(action)
}

FadeTo.clean = function() {
    caches.length = 0
}
