...
class Sprite extends Base {
    ...
    remove() {
        for (let i = 0, j = this.onRemove.length; i < j; i++) {
            this.onRemove[i].call(this)
        }
        super.remove()
        this.onUpdate.remove()
        this.afterUpdate.remove()
        this.onRemove.remove()
    }
    update(session, camera) {
        for (let i = 0, j = this.onUpdate.length; i < j; i++) {
            const onUpdate = this.onUpdate[i]
            onUpdate.call(this, session, camera)
        }
        this.updateMatrix()
        ...
        for (let i = 0, j = this.afterUpdate.length; i < j; i++) {
            const afterUpdate = this.afterUpdate[i]
            afterUpdate.call(this, session, camera)
        }
        return this
    }
